...
//    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (!"Beast!".equals(input)) {
            String[] animalTokens = bfr.readLine().split("\\s+");
            boolean isInputValid = true;

            for (String animalToken : animalTokens) {
                if (animalToken.trim().isEmpty()) {
                    System.out.println("Invalid input!");
                    isInputValid = false;
                    break;
                } else if (Integer.parseInt(animalTokens[1]) <= 0) {
                    System.out.println("Invalid input!");
                    isInputValid = false;
                    break;
                }
            ...
            switch (input) {
                case "Dog":
                    Animal dog = new Dog(animalTokens[0], Integer.parseInt(animalTokens[1]), animalTokens[2]);
                ...
                case "Cat":
                    Animal cat = new Cat(animalTokens[0], Integer.parseInt(animalTokens[1]), animalTokens[2]);
                    System.out.println(cat.toString());
                    break;

                case "Frog":
                    Animal frog = new Frog(animalTokens[0], Integer.parseInt(animalTokens[1]), animalTokens[2]);
                    System.out.println(frog.toString());
                    break;

                case "Kitten":
                    Animal kitten = new Kitten(animalTokens[0], Integer.parseInt(animalTokens[1]));
                    System.out.println(kitten.toString());
                    break;

                case "Tomcat":
                    Animal tomcat = new Kitten(animalTokens[0], Integer.parseInt(animalTokens[1]));
