...
//    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        String[] busTokens = bfr.readLine().split("\\s+");
                ...
                case "Drive":
                    if (tokens[1].equals("Car")) {
                        if(car.drive(Double.parseDouble(tokens[2]))) {
                            System.out.println("Car travelled " + (format.format(Double.parseDouble(tokens[2])) + " km"));
                        } else {
                            System.out.println("Car needs refueling");
                        }
                    } else if (tokens[1].equals("Truck")) {
                        if (truck.drive(Double.parseDouble(tokens[2]))) {
                            System.out.println("Truck travelled " + (format.format(Double.parseDouble(tokens[2])) + " km"));
                        } else {
                            System.out.println("Truck needs refueling");
                        }
                    } else if (tokens[1].equals("Bus")) {
                        bus.setFuelConsumption(bus.getFuelConsumption() + 1.4);
                        if (bus.drive(Double.parseDouble(tokens[2]))) {
                            System.out.println("Bus travelled " + (format.format((Double.parseDouble(tokens[2]))) + " km"));
                        } else {
        ...
        System.out.println((String.format("Car: %.2f%nTruck: %.2f%nBus: %.2f", car.getFuelQuantity(), truck.getFuelQuantity(), bus.getFuelQuantity())));
    }
}
